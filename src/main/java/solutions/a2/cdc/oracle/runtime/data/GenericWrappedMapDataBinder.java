 ...
 * 
 */
public class GenericWrappedMapDataBinder extends GenericAbstractMapDataBinder {


	public GenericWrappedMapDataBinder(final OraRdbmsInfo rdbmsInfo, final OraCdcTableBase table) {
	...
	public void insert(OraCdcColumn column, Object value) {
		if (column.partOfPk())
			keyData.put(column.getColumnId(), Optional.of(value));
		else
			...
			switch (column.jdbcType()) {
				case CHAR, VARCHAR, NCHAR, NVARCHAR -> {
					...
					decoder = column.encrypted()
							? GenericDecoders.getWrapped(charset, table.decrypter(), column.salted())
							: GenericDecoders.getWrapped(charset);
				}
				...
				case BINARY, NUMERIC, INTERVALYM, INTERVALDS, ROWID ->
					decoder = column.encrypted()
							? GenericDecoders.getWrapped(table.decrypter(), column.salted())
							: GenericDecoders.getWrapped();
				case FLOAT -> 
					decoder = column.IEEE754()
							? column.encrypted()
									? GenericDecoders.getWrapped(BINARY_FLOAT, table.decrypter(), column.salted())
									: GenericDecoders.getWrapped(BINARY_FLOAT)
							: decoderFromJdbcType(column);
				case DOUBLE -> 
					decoder = column.IEEE754()
							? column.encrypted()
									? GenericDecoders.getWrapped(BINARY_DOUBLE, table.decrypter(), column.salted())
									: GenericDecoders.getWrapped(BINARY_DOUBLE)
							: decoderFromJdbcType(column);
				case DECIMAL ->
					decoder = column.encrypted()
							? GenericDecoders.getWrappedNUMBER(column.dataScale(), table.decrypter(), column.salted())
							: GenericDecoders.getWrappedNUMBER(column.dataScale());
	...
	public KeyValuePair changeVector(OraCdcTransaction transaction, Map<String, Object> offset, boolean skipRedoRecord) throws SQLException {
		return skipRedoRecord
				? null
				: new KeyValuePair(stmt.getOperation(), keyData, valueData);
	}

	@Override
	public DataBinder newInstance() {
		return new GenericWrappedMapDataBinder(this.rdbmsInfo, this.table);
	}

	private OraCdcDecoder decoderFromJdbcType(final OraCdcColumn column) {
		return column.encrypted()
				? GenericDecoders.getWrapped(column.jdbcType(), table.decrypter(), column.salted())
				: GenericDecoders.getWrapped(column.jdbcType());
	}
}
