...
where TC.OWNER=IC.TABLE_OWNER and TC.TABLE_NAME=IC.TABLE_NAME and IC.COLUMN_NAME=TC.COLUMN_NAME
  and IC.INDEX_OWNER=FL.OWNER and IC.INDEX_NAME=FL.INDEX_NAME and IC.CON_ID=TC.CON_ID and IC.CON_ID=FL.CON_ID;
	...
	/*
select DBID, NAME, DB_UNIQUE_NAME, PLATFORM_NAME, OPEN_MODE, CONTROLFILE_TYPE,
       SUPPLEMENTAL_LOG_DATA_MIN, SUPPLEMENTAL_LOG_DATA_ALL, LOG_MODE, 
...
where  L.STATUS = 'ACTIVE'
  ...
  and  rownum = 1
	*/
	...
	/*
select   C.OWNER, C.CONSTRAINT_NAME, DCC.COLUMN_NAME
from     DBA_CONSTRAINTS C, DBA_CONS_COLUMNS DCC
where    C.OWNER=DCC.OWNER AND C.TABLE_NAME=DCC.TABLE_NAME and C.CONSTRAINT_NAME=DCC.CONSTRAINT_NAME and C.CONSTRAINT_TYPE='U'
  and    C.OWNER='SCOTT' and C.TABLE_NAME='DEPT'
