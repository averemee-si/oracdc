...
from   ALL_TAB_COLUMNS C
where  C.OWNER='SCOTT' and C.TABLE_NAME='DEPT'
  and  (C.DATA_TYPE in ('DATE', 'FLOAT', 'NUMBER', 'BINARY_FLOAT', 'BINARY_DOUBLE', 'RAW', 'CHAR', 'NCHAR', 'VARCHAR2', 'NVARCHAR2') or C.DATA_TYPE like 'TIMESTAMP%');
	 */
...
select VERSION, INSTANCE_NUMBER, INSTANCE_NAME, HOST_NAME,
       (select CPU_CORE_COUNT_CURRENT from V$LICENSE) CPU_CORE_COUNT_CURRENT
from   V$INSTANCE;
	 */
...
/*
select O.OWNER, O.OBJECT_NAME TABLE_NAME, O.OBJECT_ID
from   DBA_OBJECTS O
...
/*
select O.OWNER, O.OBJECT_NAME TABLE_NAME, P.PDB_NAME, O.OBJECT_ID, O.CON_ID
