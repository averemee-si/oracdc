...
== Memory management
*solutions.a2.cdc.oracle.OraCdcRedoMinerConnector* supports both https://docs.oracle.com/en/java/javase/25/core/heap-and-heap-memory.html[On-Heap and Off-Heap memory] (the latter is the default mode). To specify the type of memory used, you need to set *a2.transaction.implementation* parameter to the appropriate value:

* *OffHeap* - default value, this mode uses https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/lang/foreign/MemorySegment.html[MemorySegment] to store Oracle Database transactions in Off-Heap memory.
...
|a2.transaction.implementation | OffHeap | Mandatory setting
...
== Using a physical standby database as a source of change information.

With default settings, *solutions.a2.cdc.oracle.OraCdcRedoMinerConnector* works with primary Oracle Database instance in the *READ WRITE* state (https://docs.oracle.com/en/database/oracle/oracle-database/26/refrn/V-DATABASE.html[V$DATABASE.OPEN_MODE='READ WRITE']). If you are using https://www.oracle.com/database/data-guard/[Oracle Data Guard] and need to avoid any additional load on the primary database, you can use a standby database in the https://docs.oracle.com/en/database/oracle/oracle-database/26/refrn/V-DATABASE.html[V$DATABASE.OPEN_MODE in ('MOUNTED', 'READ ONLY', 'READ ONLY WITH APPLY')] state to capture change information. In this case, the connection to the primary database is used only to read information about table column data types, and an additional connection to the standby database is used to query the current database SCN. It's important to understand that eliminating the additional load on the primary database when reading changes from the standby database increases the latency during change processing, as the system receives these changes indirectly, only after they are transferred from the primary server to the standby.  Using asynchronous redo transport https://docs.oracle.com/en/database/oracle/oracle-database/26/dgbkr/logxptmode.html[LogXptMode=ASYNC](https://docs.oracle.com/en/database/oracle/oracle-database/26/sbydb/oracle-data-guard-protection-modes.html[Maximum Performance protection mode]) makes this latency unpredictable, while using https://docs.oracle.com/en/database/oracle/oracle-database/26/dgbkr/logxptmode.html[LogXptMode=FASTSYNC](https://docs.oracle.com/en/database/oracle/oracle-database/26/sbydb/oracle-data-guard-protection-modes.html[Maximum Availability protection mode]) or https://docs.oracle.com/en/database/oracle/oracle-database/26/dgbkr/logxptmode.html[LogXptMode=SYNC] redo transport (https://docs.oracle.com/en/database/oracle/oracle-database/26/sbydb/oracle-data-guard-protection-modes.html[Maximum Protection protection mode]) minimizes processing latency.

To use reading changes from a standby database, you need to set the following parameters

[width="100%",cols="<20%,<10%,<70%",options="header",]
|===
|Parameter Name |Value |Description

|a2.standby.activate | true | Mandatory setting.

|a2.standby.jdbc.url| String| Mandatory setting. JDBC URL to connect to the https://docs.oracle.com/en/database/oracle/oracle-database/26/sbydb/introduction-to-oracle-data-guard-concepts.html#GUID-C49AC6F4-C89B-4487-BC18-428D65865B9A[Physical Standby Database]. For information on JDBC URL syntax, please see description of the `a2.jdbc.url` parameter above

|a2.standby.wallet.location| String| Mandatory setting. Location of Oracle Wallet/https://docs.oracle.com/en/database/oracle/oracle-database/26/dbseg/configuring-authentication.html#GUID-2419D309-5874-4FDC-ADB7-65D5983B2053[External Password Store] for connecting to physical standby database with https://docs.oracle.com/en/database/oracle/oracle-database/23/refrn/V-DATABASE.html[V$DATABASE.OPEN_MODE = MOUNTED]

|a2.standby.privilege| Default - https://docs.oracle.com/en/database/oracle/oracle-database/26/dbseg/configuring-privilege-and-role-authorization.html#GUID-5798F976-85B2-4973-92F7-DB3F6BC9D497[sysdg]| The privilege used to connect to the physical standby database with https://docs.oracle.com/en/database/oracle/oracle-database/23/refrn/V-DATABASE.html[V$DATABASE.OPEN_MODE = MOUNTED]. Can be https://docs.oracle.com/en/database/oracle/oracle-database/26/dbseg/configuring-privilege-and-role-authorization.html#GUID-5798F976-85B2-4973-92F7-DB3F6BC9D497[sysdg] or https://docs.oracle.com/en/database/oracle/oracle-database/26/dbseg/configuring-privilege-and-role-authorization.html#GUID-BF12E37F-4606-42BB-B8B6-4CDC5A870EE7[sysbackup] or https://docs.oracle.com/en/database/oracle/oracle-database/26/dbseg/configuring-privilege-and-role-authorization.html#GUID-BD5D39D1-DBFF-400A-8645-355F8FB9CD31[sysdba].
...
|a2.ssh.provider | sshj, mina, or maverick | Optional, library that provides SSH connection: `maverick` for https://jadaptive.com/[Maverick Synergy] or `sshj` for https://github.com/hierynomus/sshj[Hierynomus sshj] or `mina` for https://mina.apache.org/sshd-project/[Apache Mina SSHD]. Default - `sshj`
...
|a2.ssh.connect.timeout | Integer | SSH connect timeout in ms when using https://mina.apache.org/sshd-project/[Apache Mina SSHD]. Default - 30_000

|===

*IMPORTANT* You must specify a value for one of the parameters a2.ssh.private.key or a2.ssh.password

*IMPORTANT* The libraries required to work with https://jadaptive.com/[Maverick Synergy] when `a2.ssh.provider`=`maverick` are not provided. For a commercial license for https://jadaptive.com/[Maverick Synergy], please contact https://jadaptive.com/[Jadaptive Limited]. To use https://jadaptive.com/[Maverick Synergy] licensed under https://www.gnu.org/licenses/lgpl-3.0.html[LGPL-3.0], use the commands below to download and then add these libraries to the Java virtual machine's CLASSPATH

----
SSH_VERSION=3.1.4
wget https://repo1.maven.org/maven2/com/sshtools/maverick-base/${SSH_VERSION}/maverick-base-${SSH_VERSION}.jar
wget https://repo1.maven.org/maven2/com/sshtools/maverick-logging/${SSH_VERSION}/maverick-logging-${SSH_VERSION}.jar
wget https://repo1.maven.org/maven2/com/sshtools/maverick-utils/${SSH_VERSION}/maverick-utils-${SSH_VERSION}.jar
wget https://repo1.maven.org/maven2/com/sshtools/maverick-synergy-client/${SSH_VERSION}/maverick-synergy-client-${SSH_VERSION}.jar
wget https://repo1.maven.org/maven2/com/sshtools/maverick-synergy-common/${SSH_VERSION}/maverick-synergy-common-${SSH_VERSION}.jar
----
...
You also need to add permissions to execute packages https://docs.oracle.com/en/database/oracle/oracle-database/26/arpls/DBMS_FILE_TRANSFER.html[DBMS_FILE_TRANSFER] (only procesure https://docs.oracle.com/en/database/oracle/oracle-database/26/arpls/DBMS_FILE_TRANSFER.html#GUID-ECC02A66-FAFD-4085-841C-E0B0A988E957[COPY_FILE] is used) and https://docs.oracle.com/en/database/oracle/oracle-database/26/arpls/UTL_FILE.html[UTL_FILE] (only procedure https://docs.oracle.com/en/database/oracle/oracle-database/26/arpls/UTL_FILE.html#GUID-09B09C2A-2C21-4F70-BF04-D0EEA7B59CAF[FREMOVE] is used), for example

----
grant execute on DBMS_FILE_TRANSFER to <a2.jdbc.username>;
grant execute on UTL_FILE to <a2.jdbc.username>;
----
...
= Selecting a Kafka topic for publishing changes

To select a Kafka topic to publish changes to, you must set the parameter

[width="100%",cols="<20%,<10%,<70%",options="header",]
|===
|Parameter Name |Value |Description

|a2.topic.mapper | String | The fully qualified name of the class that defines the Kafka topic to which data from the tables should be sent. The class must implement the https://github.com/averemee-si/oracdc/blob/feature/advanced-compression/src/main/java/solutions/a2/cdc/oracle/runtime/config/KafkaTopicNameMapper.java[solutions.a2.cdc.oracle.runtime.config.KafkaTopicNameMapper] interface.

|===

The connector comes with three predefined mappers: `solutions.a2.cdc.oracle.runtime.config.KafkaDefaultTopicNameMapper`, `solutions.a2.cdc.oracle.runtime.config.KafkaSingleTopicNameMapper`, and `solutions.a2.cdc.oracle.runtime.config.KafkaFlexibleTopicNameMapper`.

== a2.topic.mapper=solutions.a2.cdc.oracle.runtime.config.KafkaDefaultTopicNameMapper

This is the default value for the `a2.topic.mapper` parameter. In this mode, a unique name is generated for each processed table based on the table's PDB name, table's owner(schema name), and the table name itself. Additional parameters listed below are used to customize the topic name

[width="100%",cols="<20%,<10%,<70%",options="header",]
|===
|Parameter Name |Value |Description

|a2.topic.prefix | String | prefix to prepend to the unique name of the Kafka topic

|a2.topic.name.style | `TABLE`, `SCHEMA_TABLE`, `PDB_SCHEMA_TABLE` | `TABLE` is default

|a2.topic.name.delimiter | `_` or `-` or `.` | `_` is default


|===


== a2.topic.mapper=solutions.a2.cdc.oracle.runtime.config.KafkaSingleTopicNameMapper

In this mode, all change events for all tables are written to a single Kafka topic specified by the parameter

[width="100%",cols="<20%,<10%,<70%",options="header",]
|===
|Parameter Name |Value |Description

|a2.kafka.topic | String | Valid Apache Kafka topic name

|===


== a2.topic.mapper=solutions.a2.cdc.oracle.runtime.config.KafkaFlexibleTopicNameMapper

This mode extends `solutions.a2.cdc.oracle.runtime.config.KafkaDefaultTopicNameMapper`, providing the ability to put data for tables defined by the parameter `a2.map.topic.<TOPIC_NAME>=<LIST_OF_TABLES>` into a single topic `<TOPIC_NAME>`. Table names in the comma-separated list of tables must be in the format <OWNER>.<TABLE_NAME> or <PDB_NAME>.<OWNER>.<TABLE_NAME>. Additional parameters listed below are used to customize the topic name
...
|a2.map.topic.<TOPIC_NAME> | LIST_OF_TABLES> | 
...
| a2.tde.wallet.password | String | Password for Oracle Wallet
|===
