image:https://img.shields.io/:license-apache-blue.svg[License,link=https://raw.githubusercontent.com/averemee-si/oracdc/refs/heads/main/LICENSE]
...
*solutions.a2.cdc.oracle.runtime.thread.KafkaSourceSnapshotLogConnector* reads changes from Oracle Database materialized view logs. While it demonstrates Oracle capabilities, it is not recommended for production use due to the very large load it creates on the database. *solutions.a2.cdc.oracle.runtime.thread.KafkaSourceSnapshotLogConnector* documentation is available at - link:doc/SnapshotLogConnector.adoc[SnapshotLogConnector.adoc].
...
Minimum supported version https://openjdk.org/projects/jdk/21/[Java 21]. To run using this version, you need to pass the following arguments to the JVM

----
--enable-preview --enable-native-access=ALL-UNNAMED
----

It is recommended to use https://openjdk.org/projects/jdk/25/[Java 25]. To run using this version, you need to pass the following arguments to the JVM
...
1. Fix column shift for RDBMS tables containing guard columns (hidden column with a value for the corresponding functional index)

2. Security fixes in Dockerfile 

=== 2.15.0 (APR-2026)

1. Replacement of https://chronicle.software/queue/[Chronicle Queue] off-heap memory support to https://docs.oracle.com/en/java/javase/25/core/foreign-function-and-memory-api.html[Java's native foreign memory API]

2. In addition to the already supported SSР/SFTP connection methods using https://github.com/hierynomus/sshj[SSHJ] and https://jadaptive.com/java-ssh-library/[Maverick Synergy], support for https://mina.apache.org/sshd-project/[Apache Mina SSH] has been added

3. Bug fixes

=== 2.14.5 (MAR-2026)

1. Dockerfile: fix for https://github.com/advisories/GHSA-72hv-8253-57qq[GHSA-72hv-8253-57qq] and https://nvd.nist.gov/vuln/detail/CVE-2026-1605[CVE-2026-1605]

2. New strategy for specifying a Kafka topic by table name **solutions.a2.cdc.oracle.runtime.config.KafkaFlexibleTopicNameMapper**, for details, see link:doc/RedoMinerConnector.adoc[RedoMinerConnector.adoc]
